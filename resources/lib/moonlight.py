...
def list_games(host):
    ...
    games = _parse_game_list(result.stdout)
    _warn_if_duplicates(games)
    return True, games


def _warn_if_duplicates(games):
    """
    Si Sunshine tiene dos apps con el mismo nombre (p.ej. una añadida a mano
    con la ruta directa al .exe y otra auto-detectada de Steam), 'moonlight
    stream' recibe un nombre ambiguo y puede lanzar la que no toca. Esto solo
    avisa en el log; el arreglo real es renombrar/borrar el duplicado en
    Sunshine (https://<host>:47990 > Applications).
    """
    seen = set()
    dupes = set()
    for g in games:
        ...
        seen.add(g)
    if dupes:
        ku.log('Aviso: nombres duplicados en Sunshine (pueden lanzar la app '
               'equivocada al hacer stream por nombre): ' + ', '.join(sorted(dupes)),
               xbmc.LOGWARNING)
